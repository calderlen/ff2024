...
def feature_engineering(df):
    ...
    #df['team_year'] = df['recent_team'] + '_' + df['season'].astype(str)
    #df['opponent_year'] = df['opponent_team'] + '_' + df['season'].astype(str)
...
def clean_data(df):
    ...
    df = pd.get_dummies(df, columns=['player_id','season'])
...
def normalize_sequences(X_train, X_test):
    ...
    X_train_numeric = X_train.reshape(-1, X_train.shape[2]).astype(float)
    X_test_numeric = X_test.reshape(-1, X_test.shape[2]).astype(float)
    
    # Fit scaler on the training data only
    scaler.fit(X_train_numeric)
    
    # Transform both training and test data
    X_train_normalized = scaler.transform(X_train_numeric).reshape(X_train.shape)
    X_test_normalized = scaler.transform(X_test_numeric).reshape(X_test.shape)
    
    return X_train_normalized, X_test_normalized

def generate_cv_splits(X, y, tscv):
